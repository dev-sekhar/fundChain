...
function updateStatus(row) {
    ...
    statusBubble.classList.remove('status-pending', 'status-in-progress', 'status-completed');
    ...
    } else if (codingCheckbox.checked || testingCheckbox.checked || deploymentCheckbox.checked) {
        ...
        group.requirements.forEach((req, reqIndex) => {
            ...
            if (componentCount > 0) {
                ...
                mainRow.innerHTML = `
                    ...
                    <td class="checkbox-col"><input type="checkbox" class="deployment" data-component="${req.components[0]}" disabled /></td>
                    ...
                    <td class="date-col"><input type="date" class="actual-date" value="" max="${new Date().toISOString().split('T')[0]}" /></td>
                `;
